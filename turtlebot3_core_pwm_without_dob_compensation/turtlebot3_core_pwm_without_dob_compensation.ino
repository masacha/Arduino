...
std_msgs::Float64 measured_current;
ros::Publisher measured_current_pub("measured_current",&measured_current);
...
/*******************************************************************************
* Declaration for Current Sensor
*******************************************************************************/
...
void setup()
{
  ...
  nh.advertise(measured_current_pub);
...
{ 
  ...
  if ((millis()-tTime[4]) >= (1000 / CURRENT_PERIOD))
  {
    publishCurrent();
    tTime[4] = millis();
  }
...
void publishCurrent(void)
{
//  current_measurement = analogRead(analogInPin);
  measured_current.data = current_measurement;
  measured_current_pub.publish(&measured_current); 
}
