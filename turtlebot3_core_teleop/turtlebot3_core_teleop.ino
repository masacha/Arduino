...
void commandVelocityCallback(const geometry_msgs::Twist& cmd_vel_msg)
{
  ...
  cmd_angular_velocity = cmd_vel_msg.angular.z;
}
...
{ 
  ...
  mobile_robot_reaction_force = mobile_robot_reaction_force_tmp - G_ROBOT*(M_R)*last_linear_velocity;

  if (mobile_robot_reaction_force>RF_EPSILON){
    mobile_robot_reaction_force_tmp = mobile_robot_reaction_force_tmp + ((reaction_torque_[LEFT] + reaction_torque_[RIGHT])/WHEEL_RADIUS + G_ROBOT*(M_R)*last_linear_velocity - F_R_PLUS - D_R_PLUS*last_linear_velocity - mobile_robot_reaction_force_tmp)*G_ROBOT*step_time;
  }
  else if (mobile_robot_reaction_force<-RF_EPSILON){
    mobile_robot_reaction_force_tmp = mobile_robot_reaction_force_tmp + ((reaction_torque_[LEFT] + reaction_torque_[RIGHT])/WHEEL_RADIUS + G_ROBOT*(M_R)*last_linear_velocity - F_R_MINUS - D_R_MINUS*last_linear_velocity - mobile_robot_reaction_force_tmp)*G_ROBOT*step_time;
  }
  else{
    mobile_robot_reaction_force_tmp = mobile_robot_reaction_force_tmp + ((reaction_torque_[LEFT] + reaction_torque_[RIGHT])/WHEEL_RADIUS + G_ROBOT*(M_R)*last_linear_velocity - mobile_robot_reaction_force_tmp)*G_ROBOT*step_time;
  }
...
bool controlAcceleration(double diff_time)
{
  ...
  err_left = cmd_velocity_left - last_velocity_[LEFT];
  err_right = cmd_velocity_right - last_velocity_[RIGHT];
